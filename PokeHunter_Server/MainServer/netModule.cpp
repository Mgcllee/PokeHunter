...
Session::Session() : remain_packet_size(0), socket(NULL) {}
Session::~Session() { closesocket(socket); }

bool Session::recycle_session() { return false; }
...
void Session::set_socket(SOCKET new_socket) { socket = new_socket; }

int Session::get_prev_remain_packet_size() { return remain_packet_size; }
...
void Session::send_packet(void* packet) {
  ...
  OverlappedExpansion* sendoverlapped =
      new OverlappedExpansion{reinterpret_cast<char*>(packet)};
  WSASend(socket, &sendoverlapped->wsa_buffer, 1, 0, 0,
          &sendoverlapped->overlapped, 0);
}

void Session::disconnect() { closesocket(socket); }
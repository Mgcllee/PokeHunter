...
USER_DB_MANAGER::USER_DB_MANAGER(const char* DB_HOST, const char* DB_USER,
  ...
  conn = mysql_real_connect(&connection, DB_HOST, DB_USER, DB_PASS, DB_NAME,
                            3306,         // port_number
                            (char*)NULL,  // unix_socket
                            0);           // client_flag
}
USER_DB_MANAGER::~USER_DB_MANAGER() { mysql_close(conn); }
void USER_DB_MANAGER::test_mysql_function() {
  MYSQL_RES* result = nullptr;
  MYSQL_ROW row = NULL;

  process_sql(
      "SELECT GROUP_CONCAT(col_name separator ', ') as name FROM(SELECT "
      "COLUMN_NAME as col_name FROM INFORMATION_SCHEMA.COLUMNS WHERE  "
      "TABLE_NAME = 'user_info_table') as field_name;");
...
bool USER_DB_MANAGER::get_login_user_data(int& user_id) {
  if (0 == user_id) user_id = get_new_uid();

...
Party::Party() : member_count(0), in_stage(false), name("Empty") {}
...
bool Party::enter_member(int player_ticket, char* packet) {
  ...
  if (member_count >= PARTY_MAX_NUM) return false;

  if (0 <= member_count && member_count <= 3) {
    /*strncpy_s(parties[party_number].member[cur_party_member_count].name,
    CHAR_SIZE, players->get_player(player_ticket)->name, CHAR_SIZE);
    parties[party_number].member[cur_party_member_count].user_id = user_id;

    strncpy_s(parties[party_number].member[cur_party_member_count]._pet_num,
    CHAR_SIZE, players->get_player(player_ticket)->_pet_num, CHAR_SIZE);
    {
            std::lock_guard<std::mutex> ll{
    players->get_player(player_ticket)->_lock };
            parties[party_number].member[cur_party_member_count]._player_state =
    ST_NOTREADY; players->get_player(player_ticket)->_player_state =
    ST_NOTREADY;
    }
    ...
    for (Session& cl :
    parties[players->get_player(player_ticket)->_party_num].member) { if (0 ==
    strcmp("None", cl.name)) continue; else  if (-1 == cl.user_id)
    continue;
    ...
    strncpy_s(in_party._myname, CHAR_SIZE,
    players->get_player(player_ticket)->name,
    strlen(players->get_player(player_ticket)->name));
    players->get_player(player_ticket)->do_send(&in_party);
...
bool Party::leave_member(char* memname) {
  ...
  for (Player* mem : member) {
    if (0 == strcmp(mem->get_user_name(), memname)) {
      member.remove(mem);
      if (0 == --member_count) in_stage = false;
...
bool Party::get_party_in_stage() { return in_stage; }

short Party::get_member_count() { return member_count; }